...
pub fn polygon_intersects_bbox(polygon: &Vec<Vec<f64>>, bbox: &[f64]) -> bool {
    ...
    if poly_max_lng < min_lng
        || poly_min_lng > max_lng
        || poly_max_lat < min_lat
        || poly_min_lat > max_lat
    {
        return false;
    }
...
fn line_segments_intersect(p1: &[f64], p2: &[f64], p3: &[f64], p4: &[f64]) -> bool {
    ...
    if d1 == 0.0 && is_point_on_segment(p3, p4, p1) {
        return true;
    }
    if d2 == 0.0 && is_point_on_segment(p3, p4, p2) {
        return true;
    }
    if d3 == 0.0 && is_point_on_segment(p1, p2, p3) {
        return true;
    }
    if d4 == 0.0 && is_point_on_segment(p1, p2, p4) {
        return true;
    }
